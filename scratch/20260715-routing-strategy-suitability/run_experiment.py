...
def artifact_gate(row: dict, case_dir: Path) -> tuple[bool, str]:
    ...
    runlog = case_dir / "runlog"
    port_patterns = []
    if row["topology_profile"].startswith("clos-"):
        port_patterns.extend(("PortTrace_node_32_port_*.tr*", "PortTrace_node_33_port_*.tr*"))
    else:
        port_patterns.append("PortTrace_node_2_port_*.tr*")
    if not any(any(runlog.glob(pattern)) for pattern in port_patterns):
        return False, "branch port trace missing"
    if row["observability"] == "detailed" and row["transport"] == "RTP":
        if not any(runlog.glob("AllPacketTrace_PKT_node_*.tr*")):
...
def pilot_rows(rows: list[dict]) -> list[dict]:
    control = next((row for row in rows if row["role"] == "control"), rows[0])
    if rows[0]["block_id"] == "adaptive-signal":
        treatment = next(row for row in rows if row["selector"] == "ADAPTIVE")
    elif rows[0]["block_id"] == "ingress-entropy":
        treatment = next(row for row in rows if row["selector"] == "INGRESS_PORT_STRIPE")
    else:
        treatment = next((row for row in rows if row["case_id"] != control["case_id"]), control)
    return list({row["case_id"]: row for row in (control, treatment)}.values())
...
def main() -> None:
    ...
    for block in BLOCK_ORDER:
        ...
        for index, row in enumerate(remaining, start=1):
            if index % 10 == 1 and package_size_bytes() > MAX_PACKAGE_BYTES:
                checkpoints[block] = "failed: package-size safety limit exceeded"
            ...
            if result["status"] != "success":
                ...
                results.extend(skipped_result(item, "block_safety_stop") for item in tail)
                break
        else:
    ...
    if stop_all:
        ...
        results.extend(
            skipped_result(row, "global_safety_stop")
            for row in MATRIX
