#!/usr/bin/env python3
"""Run three delay-jitter replicas for all requested test91/test92 cases."""
...
TARGET_ROOT = POD_ROOT / "test91_92_delay_jitter_three_groups"
...
def load_traffic(test_name: str) -> tuple[list[str], list[dict[str, str]]]:
    source = POD_ROOT / test_name / "case01_standard" / "traffic.csv"
    with source.open(encoding="utf-8-sig", newline="") as handle:
        reader = csv.DictReader(handle)
        fields = list(reader.fieldnames or [])
        rows = list(reader)
    if {"taskId", "delay"} - set(fields):
        raise ValueError(f"{source} must contain taskId and delay")
    return fields, rows


def jitter_traffic(rows: list[dict[str, str]], seed: int) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    for row in rows:
        ...
        updated = dict(row)
        updated["delay"] = f"{base_ps + jitter_ps}ps"
        ...
        audit.append(
            {
                ...
                "jittered_delay": updated["delay"],
            }
        )
    return jittered, audit


def prepare(log_dir: Path) -> list[tuple[str, str, int, str, str, Path]]:
    ...
    for test_name, test_title in TESTS:
        fields, traffic = load_traffic(test_name)
        for seed in GROUP_SEEDS:
            jittered, audit = jitter_traffic(traffic, seed)
            write_csv(
                log_dir / f"{test_name}__delay_jitter_seed{seed}.csv",
                ["taskId", "base_delay", "jitter_ps", "jittered_delay"],
                audit,
            )
            for case_name, case_title in CASES:
                ...
                target = TARGET_ROOT / f"group_seed{seed}" / test_name / case_name
...
def prepared_jobs() -> list[tuple[str, str, int, str, str, Path]]:
    ...
    for test_name, test_title in TESTS:
        for seed in GROUP_SEEDS:
            for case_name, case_title in CASES:
                target = TARGET_ROOT / f"group_seed{seed}" / test_name / case_name
                if not (target / "traffic.csv").is_file():
                    raise FileNotFoundError(f"missing prepared traffic: {target / 'traffic.csv'}")
                jobs.append((test_name, test_title, seed, case_name, case_title, target))
    return jobs
...
def launch(
    seed: int, target: Path, log_path: Path
) -> tuple[subprocess.Popen[str], object, float]:
...
def write_summaries(rows: list[dict[str, str]], log_dir: Path) -> None:
    fields = (
        "test", "test_title", "seed", "rng_seed", "case", "title", "returncode", "elapsed_s", "status", "tasks",
    ...
    baseline_map = {(row["test"], row["seed"]): row for row in rows if row["case"] == "case01_standard"}
    for row in rows:
        baseline = baseline_map.get((row["test"], row["seed"]))
...
def run_jobs(jobs: list[tuple[str, str, int, str, str, Path]], parallel: int, log_dir: Path) -> int:
    pending = jobs[:]
    active: list[tuple[str, str, int, str, str, Path, subprocess.Popen[str], object, float, Path]] = []
    rows: list[dict[str, str]] = []
    while pending or active:
        while pending and len(active) < parallel:
            test_name, test_title, seed, case_name, case_title, target = pending.pop(0)
            log_path = log_dir / f"{test_name}__seed{seed}__{case_name}.log"
            process, handle, started = launch(seed, target, log_path)
            active.append((test_name, test_title, seed, case_name, case_title, target, process, handle, started, log_path))
            print(f"[{datetime.now():%H:%M:%S}] START {test_name} seed={seed} {case_name}", flush=True)
        time.sleep(5)
        remaining = []
        for test_name, test_title, seed, case_name, case_title, target, process, handle, started, log_path in active:
            result = process.poll()
            if result is None:
                remaining.append((test_name, test_title, seed, case_name, case_title, target, process, handle, started, log_path))
                continue
            handle.close()
            row = {
                "test": test_name,
            ...
            rows.append(row)
            print(f"[{datetime.now():%H:%M:%S}] DONE {test_name} seed={seed} {case_name} rc={result}", flush=True)
        active = remaining
...
def main() -> int:
    ...
    parser.add_argument("--run-prepared", action="store_true", help="run the existing prepared replicas without rewriting traffic")
    parser.add_argument(
        "--label",
        default=f"test91_92_delay_jitter_three_groups_{datetime.now():%Y%m%d_%H%M%S}",
    ...
    jobs = prepared_jobs() if args.run_prepared else prepare(log_dir)
    print("groups=3 seeds=101,202,303 delay=base+U[0,499]ps rng_run=group_seed", flush=True)
    print(f"jobs={len(jobs)} target_root={TARGET_ROOT}", flush=True)
