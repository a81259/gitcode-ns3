...
class UnifiedPod1dTest(unittest.TestCase):
    ...
    def test_generates_ns3_and_netisim_cases_through_existing_builders(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(root.findall(".//credit_fc"))
            for credit_ele in root.findall(".//credit_fc"):
                self.assertEqual(credit_ele.attrib["enable"], "false")
            self.assertTrue(root.findall("./dcn_common_node_config/common_node/template/cbfc_cfg"))
            host_cbfc = root.find("./dcn_common_node_config/common_host/template/cbfc_cfg/crd_grp")
            self.assertIsNotNone(host_cbfc)
            self.assertEqual(host_cbfc.attrib["port"], "0..1")
            self.assertEqual(host_cbfc.attrib["node_port_cell_txfree"], "51")
            ...
            self.assertEqual(links[1].attrib["src_node"], "0..15")
            self.assertEqual(links[1].attrib["src_port"], "1")
            self.assertEqual(links[1].attrib["dst_node"], "16..31")
            ...
            self.assertEqual(links[2].attrib["src_node"], "16")
            self.assertEqual(links[2].attrib["src_port"], "1..4")
            self.assertEqual(links[2].attrib["dst_node"], "32..35")
            self.assertEqual(links[2].attrib["dst_port"], "0")
            self.assertEqual(links[2].attrib["bandwidth"], "112")
            self.assertEqual(links[2].attrib["delay"], "10")
